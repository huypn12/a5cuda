...
A5CudaSlice::A5CudaSlice(A5Cuda *a5cuda, int deviceId, int dp,
...
void A5CudaSlice::init() {
  ...
  // huy.phung: workaround, given the fact that (1) constant memory manipulating
  // function must be in the same scope with declaration
  //      and so (2) #include -ing the declaration of constant memory here must
  //      lead to invalid symbol copy
  ...
  */
  ...
  uint4 *h_UAstates =
      (uint4 *)malloc(mDataSize * sizeof(uint4) + MEMORY_ALIGNMENT);
  hm_states = (uint4 *)ALIGN_UP(h_UAstates, MEMORY_ALIGNMENT);
  checkCudaErrors(cudaHostRegister(hm_states, mDataSize * sizeof(uint4),
                                   CU_MEMHOSTALLOC_DEVICEMAP));
  checkCudaErrors(cudaHostGetDevicePointer((void **)&d_states, hm_states, 0));

  unsigned int *h_UAcontrol = (unsigned int *)malloc(
      mDataSize * sizeof(unsigned int) + MEMORY_ALIGNMENT);
  hm_control = (unsigned int *)ALIGN_UP(h_UAcontrol, MEMORY_ALIGNMENT);
  checkCudaErrors(cudaHostRegister(hm_control, mDataSize * sizeof(unsigned int),
                                   CU_MEMHOSTALLOC_DEVICEMAP));
