 ...
 */
int A5Cuda::Submit(uint64_t start_value, unsigned int start_round,
 ...
 * @return: size of input queue
 */
int A5Cuda::SubmitPartial(uint64_t start_value, unsigned int stop_round,
                          uint32_t advance, void* context) {
  mMutex.lock();
...
/**
 * Author: Huy Phung
 * Pop a set of (startval, start round, endround, roundfunc), packaging them to
 * Jobpiece_s
 * @param: jobpiece_s
 ...
 */
bool A5Cuda::PopResult(uint64_t& start_value, uint64_t& stop_value,
                       void* context)  //, uint32_t& start_round, uint32_t&
                                       // stop_round, void** context)
{
  bool res = false;
...
int DLL_PUBLIC A5CudaSubmit(uint64_t start_value, int32_t start_round,
                            uint32_t advance, void* context) {
  if (a5Instance) {
...
int DLL_PUBLIC A5CudaSubmitPartial(uint64_t start_value, int32_t stop_round,
                                   uint32_t advance, void* context) {
  if (a5Instance) {
